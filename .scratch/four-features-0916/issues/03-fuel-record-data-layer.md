...
**Status:** ready-for-human（实现完成 + 2026-09-20 迁移修订落地：analyze 零问题 + 全量 450 测试绿。本票无 UI、无真机验收面；恢复备份清表名单与备份导出已随 04 票落地）
...
- [x] 数据库 schemaVersion 2→3，表清单注册，跑 build_runner 并检查生成物
...
- [x] 【2026-09-20 修订】onUpgrade 增量迁移 v2→v3：只建 fuel_records 新表，**存量数据保留**（ADR 0005 修订，用户拍板：升级不得删库重建）；新增 `database_migration_test` 2 用例锁政策——v2 库（削新表+降 user_version）重开触发真实迁移链路，断言存量数据不丢、新表可用；全新安装抽查建表
- [x] 【2026-09-20 修订】文档同步：ADR 0005 追加修订记录、AGENTS.md 升级策略、migration 文档"版本和升级策略"章重写、ADR 0014 两处"删库重建"表述更正
