...
- 产品/文档正式版：v1。
- Drift 数据库版本：`schemaVersion = 1`。
- JSON 备份契约：`schemaVersion = 1`。
- 车型目录 asset：`schemaVersion = 1`。

三个契约版本相互独立但起点统一（见 `docs/adr/0005`）：数据库与备份只服务全新安装，库文件版本与代码不一致时删库重建，备份解码只认当前版本，不写升级/兼容分支。数据库表、备份字段、偏好 key 和路由语义变更必须同步考虑版本号、测试和文档。
