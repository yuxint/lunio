...
- **2026-09-16 六轮（深链接入，推翻初版拍板 Q7）**：决定 5 中「点击小组件不接深链」改为**接深链落提醒页**——车主上次停在别的 tab 时点小组件还得手动找回来，热启回跳省一次点击；冷启默认首屏本就是提醒页，行为不变。实现：Runner Info.plist 注册 URL scheme `lunio`（CFBundleURLTypes，工程首批深链注册）；小组件根视图一处 `.widgetURL(URL(string: "lunio:///reminders"))` 覆盖小/中两档与空态。URL 必须是 `lunio:///reminders` 三斜杠形态：go_router 只匹配 path，双斜杠的 `lunio://reminders` 里 `reminders` 会被解析成 host、path 为空匹配失败。热启由 Flutter scene delegate（`FlutterSceneDelegate`，SceneDelegate 基类）自动转发 URL 给 go_router，Dart 路由零改动。当初"从零建 scheme 只为省一次点击不划算"的成本判断不再成立——scheme 注册是一行 plist、widgetURL 是一行修饰符，边际成本趋近于零；「深链定位到具体项目」「通知点击深链」仍不做（见四功能轮 spec Out of Scope）。
