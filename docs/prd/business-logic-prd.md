...
- 图片附件、OCR 和保养单扫描。
- 多设备实时协作。
- 保养提醒等其他业务的实时活动呈现（停车倒计时的 iOS 实时活动/灵动岛已实现，见第 8 节与 docs/adr/0012）。
...
- 保存倒计时即启动/重建实时活动，与系统通知开关无关；到点时刻已过则跳过。
- 卡片展示剩余时间数与进度条（从满条递减、到点走空 0%），不放开始/结束钟点、不放车辆名；全程零更新渲染（系统自动走时，App 不推送任何更新）。
- 到点后卡片置灰"已到期"停 00:00；App 下一次运行（回前台/冷启动/点开到点通知）对账时切成"已超时"正计时，与 App 内卡片的到期正计时同构。
- 结束倒计时、清空数据会撤掉实时活动；删车、恢复备份、关闭系统通知总开关都不影响它（实时活动在系统设置里是独立开关）。
- iOS 16.2 以下、用户在系统设置关闭实时活动、或活动数量到上限时，静默降级为"只有通知"，不提示不报错。
- 保养提醒等其他业务没有实时活动呈现。
